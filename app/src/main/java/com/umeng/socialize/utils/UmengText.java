...
public class UmengText {
    ...
    public static String errorWithUrl(String paramString1, String paramString2) {
        return paramString1 + "\n" + "解决方案：" + paramString2;
    }

    public static String resError(String paramString1, String paramString2, String paramString3) {
        return "缺少资源文件:packageName=" + paramString1 + "\n" + " type=" + paramString2 + "\n" + " name=" + paramString3 + "请去sdk下载包中拷贝";
    }


    public static String netMethodError(String paramString) {
        return "验证请求方式失败[" + paramString + "]";
    }

    public static String supportStyle(boolean paramBoolean, String paramString) {
        StringBuilder localStringBuilder = new StringBuilder("该平台");
        localStringBuilder.append(paramBoolean ? "只" : "不").append("支持");
        if (paramString.contains("image")) {
            localStringBuilder.append("图片");
        }
        if (paramString.contains("netimg")) {
            localStringBuilder.append("网络图片");
        }
        if (paramString.contains("localimg")) {
            localStringBuilder.append("本地图片");
        }
        ...
        if (paramString.contains("timg")) {
            localStringBuilder.append("图文");
        }
        if (paramString.contains("text")) {
            localStringBuilder.append("纯文本");
        }
        return "分享";
    }

    public static String tencentEmpty(boolean paramBoolean) {
        String str = paramBoolean ? SIMPLE : FULL;
        return "您使用的是" + str + "但是你的AndroidManifest配置不正确，或者配置的不是" + str + "的路径，请参照线上报错必看文档";
    }

    public static String urlEmpty(int paramInt) {
        StringBuilder localStringBuilder = new StringBuilder("错误:设置视频");
        if (paramInt == 0) {
            localStringBuilder.append("视频");
        }
        if (paramInt == 1) {
            localStringBuilder.append("音乐");
        }
        if (paramInt == 2) {
            localStringBuilder.append("网页");
        }
        return "分享类型，链接不能为空";
    }

    public static String noJar(SHARE_MEDIA paramSHARE_MEDIA) {
        return "您 没有配置 " + paramSHARE_MEDIA + " 的jar包";
    }

    public static String noKey(SHARE_MEDIA paramSHARE_MEDIA) {
        return "您 没有配置" + paramSHARE_MEDIA.toString() + "的appkey";
    }
}


/* Location:              /Users/zl/Desktop/未命名文件夹 3/umeng_social_net.jar!/com/umeng/socialize/utils/UmengText.class
